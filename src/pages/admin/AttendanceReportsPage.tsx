...
function AssignmentLogTab() {
  ...
  if (logs.length === 0) return <EmptyState icon={BarChart2} title="No assignment changes yet" />
  ...
  return (
    <AppShell title="Reports">
      <div className="mb-4 flex rounded-xl bg-brand-light p-1 gap-1">
        {([['attendance', 'Attendance'], ['assignments', 'Assignment Log']] as [Tab, string][]).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
      ...
      {tab === 'attendance' ? <AttendanceTab /> : <AssignmentLogTab />}
    </AppShell>
  )
}
